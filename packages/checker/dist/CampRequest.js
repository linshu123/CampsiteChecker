...
function SendRequest(arrivalDate, stayLength, campsite) {
    var headers = {
    ...
    var arrivalDateShortString = getDateShortString(arrivalDate);
    var departureDateShortString = getDateShortString(departureDate);
    var dataParams = {
        "contractCode": "NRSO",
        "parkId": campsite.id,
        "siteTypeFilter": "ALL",
    ...
    var dataString = querystring.stringify(dataParams);
    var options = {
...
function writeReponseToFile(fileName, content) {
...
function printAvailableSiteNumberFromHTML(htmlBody) {
    var $ = cheerio.load(htmlBody);
    var date = $('#arrivalDate').text();
    var campsiteName = $('#cgroundName').text();
    var summary = $('.matchSummary').text();
    if (summary.length > 0) {
